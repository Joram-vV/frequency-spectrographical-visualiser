...
void app_main(void)
{
  ...
  vl6180_config.reset_gpio = GPIO_NUM_19;
  ...
  {
    ESP_LOGD(TAG, "No VL6180 succesfully probed");
  }
  else if (probe_ret != ESP_OK)
  {
    ESP_LOGE(TAG, "Error whilst probing VL6180: %s", esp_err_to_name(probe_ret));
  }

  // initialize_all_vl6180();
  esp_err_t ret = vl6180_init();
  if (ret == ESP_OK)
  {
    ESP_ERROR_CHECK(vl6180_configure_default());
  }else{
    ESP_LOGE(TAG, "Error: %s", esp_err_to_name(ret));
  }

  ESP_LOGD(TAG, "Place reflecting object 50mm away...");

  int8_t offset = 0;
  ESP_ERROR_CHECK(vl6180_calibrate_offset(&offset));
  ...
  while (1)
  {
    ...
    if (ret == ESP_OK)
    {
      ESP_LOGI("VL6180", "Distances: %d", distance);
    }
