	...
	if (total_songs <= 0) ESP_LOGI(MAIN_TAG, "No songs.");
		...
		for (int i = 0; i < total_songs; i++) {
			...
			if (sdcard_list_choose(playlist, i, &url) == ESP_OK && url != NULL) {
				...
				ESP_LOGI(MAIN_TAG, "Found %s at spot %d in the playlist.", filename, i);
			}
		}
	}
}
...
static void espnow_command_task(void *pvParameters) {
	...
	ESP_LOGI(MAIN_TAG, "ESP-NOW Command Task Started");
		...
		if (espnow_transport_receive(&packet, portMAX_DELAY)) {
			...
			if (packet.type == MSG_TYPE_COMMAND) {
				...
				switch (packet.payload.command.id) {
					case CMD_PLAY_INDEX:
						ESP_LOGI(MAIN_TAG, "Network Command: PLAY INDEX %ld", packet.payload.command.value);
						// Using sdcard_list_choose from your sdcard_list.c
						sdcard_list_choose(playlist, packet.payload.command.value, &url);
						if (url) {
					...
					case CMD_NEXT:
						ESP_LOGI(MAIN_TAG, "Network Command: NEXT");
					...
					case CMD_PREVIOUS:
						ESP_LOGI(MAIN_TAG, "Network Command: PREVIOUS");
						// --- FIX 1: Used sdcard_list_prev with a positive step ---
						sdcard_list_prev(playlist, 1, &url);
						if (url) {
							audio_control_play_track(url);
							is_playing = true;
						}
					...
					case CMD_PLAY_PAUSE:
						ESP_LOGI(MAIN_TAG, "Network Command: PLAY/PAUSE");
						if (is_playing) {
							audio_control_pause();
							is_playing = false;
						} else {
					...
					case CMD_SET_VOLUME:
						ESP_LOGI(MAIN_TAG, "Network Command: VOLUME %ld", packet.payload.command.value);
					...
					default:
						ESP_LOGW(MAIN_TAG, "Unhandled network command ID: %d", packet.payload.command.id);
						break;
				}
			}
		}
...
void broadcast_playlist(void) {
	if (playlist == NULL) {
		ESP_LOGW(MAIN_TAG, "Cannot broadcast: SD card list not initialized");
		return;
	}

	int total_songs = sdcard_list_get_url_num(playlist);
	if (total_songs <= 0) {
		ESP_LOGI(MAIN_TAG, "No songs to broadcast");
		return;
	}

	ESP_LOGI(MAIN_TAG, "Broadcasting playlist (%d songs)...", total_songs);
	...
	ESP_LOGI(MAIN_TAG, "Finished broadcasting playlist.");
}
...
void app_main(void) {
	...
	// Start SD-card manager
	ESP_LOGI(MAIN_TAG, "Initialising SD-card reader.");
	init_sd_card_reader();

	// Init board audio hal
	ESP_LOGI(MAIN_TAG, "Initialising audio HAL.");
	audio_board_handle_t board_handle = audio_board_init();
	audio_hal_ctrl_codec(board_handle->audio_hal, AUDIO_HAL_CODEC_MODE_DECODE, AUDIO_HAL_CTRL_START);

	// Init audio pipeline
	ESP_LOGI(MAIN_TAG, "Initialising audio pipeline.");
	audio_control_init();

	// Init visualisation module
	ESP_LOGI(MAIN_TAG, "Initialising frequency visualisation module.");
	visualiser_init();

	// Init console
	ESP_LOGI(MAIN_TAG, "Initialising console.");
	console_init(&board_handle, &playlist);

	// Create playlist
	sdcard_list_create(&playlist);
	while (!is_sd_card_mounted()) {}
	sdcard_scan(
		sdcard_url_save_callback,
		SD_CARD_MOUNT_POINT,
		0,
		(const char*[]) {"mp3"},
		1,
	...
	// Init visualisation module
	ESP_LOGI(MAIN_TAG, "Initialising fan controller.");
	...
	ESP_LOGI(MAIN_TAG, "Starting ESP-NOW Transport");
