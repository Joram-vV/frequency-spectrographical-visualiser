...
#define NUM_BANDS 7
#define TARGET_VISUALISER_FPS 1
...
void visualiser_preprocess_file(const char* mp3_url) {
	...
	if (f_check) {
		...
		if (fscanf(f_check, "%d\n", &file_fps) == 1 && file_fps == TARGET_VISUALISER_FPS) {
	...
	const float target_window_sec = 1.0f / TARGET_VISUALISER_FPS;
...
void visualiser_task(void *pvParameters) {
	int bands[NUM_BANDS];
	int current_file_fps = TARGET_VISUALISER_FPS; // Fallback default
	TickType_t delay_ticks = pdMS_TO_TICKS(1000 / TARGET_VISUALISER_FPS);
		...
		if (vis_running && vis_file != NULL) {
			...
			if (is_new_file) {
				if (fscanf(vis_file, "%d\n", &current_file_fps) == 1) {
					if (current_file_fps <= 0) current_file_fps = TARGET_VISUALISER_FPS; // Sanity check
