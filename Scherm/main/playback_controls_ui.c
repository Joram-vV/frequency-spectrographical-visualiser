...
static char now_playing_title[64];
...
static void apply_current_song(int32_t index)
{
    ...
    now_playing_ui_set_song_title(songs[index]);
...
{
    if (is_playing) {
        now_playing_ui_set_song_title(songs[current_song_index]);
        return;
    }

    snprintf(now_playing_title, sizeof(now_playing_title), "%s - paused", songs[current_song_index]);
    now_playing_ui_set_song_title(now_playing_title);
}

static void create_song_item(lv_obj_t *parent, uint32_t index)
{
    ...
    lv_label_set_text(label, songs[index]);
...
bool playback_controls_ui_handle_touch(const lv_point_t *point)
{
    ...
    if (control_index == CONTROL_PREVIOUS) {
        current_song_index = (current_song_index + SONG_COUNT - 1) % SONG_COUNT;
        is_playing = true;
